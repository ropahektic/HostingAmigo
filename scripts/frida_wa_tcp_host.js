// Frida: capture TCP payloads for Worms Armageddon when this process is the GAME HOST.
// Place next to WA.exe (e.g. Steam WA folder) or pass absolute -l path.
//
// Tracks sockets where:
//   - accept() returns a socket whose LOCAL port is in WA_CAPTURE_PORTS (default 17011), or
//   - connect() targets REMOTE port in WA_CAPTURE_PORTS, or
...
//   WA_CAPTURE_PREFIX  — "0" disables: auto-track socket on send/recv when bytes look like a WA
//                        record (channel 0x01/0x02 + u16 length). Default ON — needed for WormNET / non-17011.
//   WA_CAPTURE_ALL     — "1" to log every send/recv on any socket (very noisy)
//   WA_CAPTURE_HEX_MAX — max bytes hex per line (default 4096)
//   WA_LOG_PATH        — full path to jsonl; default is next to the RUNNING WA.exe (Process.mainModule),
//                        not next to where the .js lives — e.g. GOG WA writes under C:\GOG Games\...
//
// Multiple WA.exe: NEVER use frida -n "WA.exe" (picks an arbitrary instance). Use PID.
// WA shows the PID in the window title, e.g. "Worms Armageddon [15968]" -> frida -p 15968 ...
//
// Prefer CMD (no ExecutionPolicy issues): run_frida_wa_tcp.cmd host 15968
// Or: powershell -NoProfile -ExecutionPolicy Bypass -File run_frida_wa_tcp.ps1 -Role host -Pid 15968
// Or: frida -p 15968 -l "C:\...\Worms Armageddon\frida_wa_tcp_host.js"
//
// Spawn single WA under Frida (only when you have zero WA running):
//   frida -f ".\WA.exe" -l ".\frida_wa_tcp_host.js"
//
// Pair: frida_wa_tcp_joiner.js on each joiner process (same script, different -p PIDs).
//
// Frida 17+: no static Module.getExportByName('x.dll','fn') — use Process.find/getModuleByName.
// Run joiner capture from a second cmd.exe (not inside Frida's "->" REPL).
'use strict';
